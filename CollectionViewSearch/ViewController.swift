...
class ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UISearchResultsUpdating, UISearchBarDelegate
{
        ...
        filteredData = data.filter({ (string) -> Bool in
            if let searchTerm = self.searchController.searchBar.text {
                ...
                if searchTermMatches {
                    var searchTermInScope = false
                    switch searchController.searchBar.selectedScopeButtonIndex {
                    case 0:
                        if string.characters.count < 7 {
                            searchTermInScope = true
                        }
                    case 1:
                        if string.characters.count >= 7 && string.characters.count < 12 {
                            searchTermInScope = true
                        }
                    case 2:
    ...
    {
        if let detailViewController = self.storyboard?.instantiateViewControllerWithIdentifier("DetailViewController") as? DetailViewController {
            var labelText: String
            if searchController.active {
                labelText = filteredData[indexPath.row] as String
            } else {
                labelText = data[indexPath.row] as String
            }
